// -*- C++ -*-
//
// Package:    HGCal/DigiPlotter_Ped
// Class:      DigiPlotter_Ped
//
/**\class DigiPlotter_Ped DigiPlotter_Ped.cc HGCal/DigiPlotter_Ped/plugins/DigiPlotter_Ped.cc
...
class DigiPlotter_Ped : public edm::one::EDAnalyzer<edm::one::SharedResources>
{

public:
	explicit DigiPlotter_Ped(const edm::ParameterSet&);
	~DigiPlotter_Ped();
...
//
DigiPlotter_Ped::DigiPlotter_Ped(const edm::ParameterSet& iConfig)
{
...
DigiPlotter_Ped::~DigiPlotter_Ped()
{
...
void
DigiPlotter_Ped::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
	using namespace edm;
	using namespace std;
                                             ...
                                             if((SKI.detid()).cellType() == 2 || (SKI.detid()).cellType() == 3 || (SKI.detid()).cellType() == 5 ){
                                                   if((iux <= 0.25 && iyy>= -0.25 ) || (iux < -0.5) )
                                                     ...
                                                     h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].adc() - (Average_Pedestal_Per_Event1_Full/(Cell_counter1_Full))) );
                                                      else if((iux > -0.25 && iyy < -0.50 ) || (iux > 0.50)) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].adc() - (Average_Pedestal_Per_Event2_Full/(Cell_counter2_Full))) );
                                                     }                                  
...
void
DigiPlotter_Ped::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
DigiPlotter_Ped::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
...
DEFINE_FWK_MODULE(DigiPlotter_Ped);
