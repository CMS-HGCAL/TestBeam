...
void PedestalPlotter::endJob()
{
  ...
  for( std::set< std::pair<int,HGCalTBDetId> >::iterator it=setOfConnectedDetId.begin(); it!=setOfConnectedDetId.end(); ++it ){
    ...
    for( size_t it=0; it<NUMBER_OF_SCA; it++ ){
      ...
      float hgMean=m_meanHGMap[key]/m_evtID;
      float lgMean=m_meanLGMap[key]/m_evtID;
      float hgRMS=std::sqrt(m_rmsHGMap[key]/m_evtID-m_meanHGMap[key]/m_evtID*m_meanHGMap[key]/m_evtID);
      float lgRMS=std::sqrt(m_rmsLGMap[key]/m_evtID-m_meanLGMap[key]/m_evtID*m_meanLGMap[key]/m_evtID);
