...
void RawHitPlotter::beginJob()
{
  ...
  for( size_t iski=0; iski<HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA; iski++ ){ 
    ...
    int skiId=(HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA-iski)%HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA+1;
    for( size_t ichan=0; ichan<HGCAL_TB_GEOMETRY::N_CHANNELS_PER_SKIROC; ichan++ ){
      HGCalTBElectronicsId eid(skiId,ichan);      
      if( !essource_.emap_.existsEId(eid) ) continue;
      int key=iski*100+ichan;
    ...
    m_h_cmHighTime.insert( std::pair<int,TH2F*>(iski, htmp2) );
    htmp2=cmdir.make<TH2F>("LowGain_RatiosVsTS","LowGain_RatiosVsTS",12,-0.5,11.5,1000,-10,10);
    m_h_cmLowTime.insert( std::pair<int,TH2F*>(iski, htmp2) );
    htmp1=cmdir.make<TH1F>("NhitUnderZero","NhitUnderZero",33,0,33);
    m_h_nhitPerSkirocUnderSat.insert( std::pair<int,TH1F*>(iski, htmp1) );
    for( size_t it=0; it<NUMBER_OF_TIME_SAMPLES; it++ ){
      os.str("");
      os << "HighGain_TS" << it ;
      htmp1=cmdir.make<TH1F>(os.str().c_str(),os.str().c_str(),4000,-500,3500);
      m_h_cmHigh.insert( std::pair<int,TH1F*>(iski*100+it, htmp1) );

      os.str("");
      os << "LowGain_TS" << it ;
      htmp1=cmdir.make<TH1F>(os.str().c_str(),os.str().c_str(),4000,-500,3500);
      m_h_cmLow.insert( std::pair<int,TH1F*>(iski*100+it, htmp1) );
    }
  }
...
void RawHitPlotter::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
  ...
  if( m_eventPlotter ){
    ...
    for( size_t it=0; it<NUMBER_OF_TIME_SAMPLES; it++ ){
      ...
      InitTH2Poly(*h);
      polyMap.insert( std::pair<int,TH2Poly*>(it,h) );
    }
  }
  ...
  for( std::map<int,commonModeNoise>::iterator it=cmMap.begin(); it!=cmMap.end(); ++it ){
	...
	m_h_cmHighTime[m_skirocID]->Fill( ts,(m_cmHigh[ts]-m_cmHigh[0])/m_cmHigh[0] );
      if( m_cmLow[ts]>-160 && m_cmLow[0]!=0 )
      m_h_cmLowTime[m_skirocID]->Fill( ts,(m_cmLow[ts]-m_cmLow[0])/m_cmLow[0] );
    }
    m_tree->Fill();
  }

  int nhitUnderSat[HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA];
  for( int i=0; i<HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA; i++ ) nhitUnderSat[i]=0;
  for( auto hit : *hits ){
    HGCalTBElectronicsId eid( essource_.emap_.detId2eid(hit.detid().rawId()) );
    if( !essource_.emap_.existsEId(eid) ) continue;
    int ichan=hit.channel();
    int iski=hit.skiroc();
    std::pair<int,HGCalTBDetId> p( (iski%HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA)*100+ichan,hit.detid() );
    setOfConnectedDetId.insert(p);
    channelInfo* cif=m_channelMap[(iski%HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA)*100+ichan];
	...
	cif->counterLGMap[it]+=1;
      ...
      polyMap[ it ]->Fill(CellCentreXY.first , CellCentreXY.second, highGain);
    }
  }
  int ntot=0;
  for( int iski=0; iski<HGCAL_TB_GEOMETRY::N_SKIROC_PER_HEXA; iski++ ) {
    ntot+=nhitUnderSat[iski];
    m_h_nhitUnderSat_VS_SkirocID->Fill(iski,nhitUnderSat[iski]);
    m_h_nhitPerSkirocUnderSat[iski]->Fill(nhitUnderSat[iski]);
  }
  m_h_nhitUnderSat->Fill(ntot);
}

void RawHitPlotter::InitTH2Poly(TH2Poly& poly)
{
  double HexX[MAXVERTICES] = {0.};
  double HexY[MAXVERTICES] = {0.};
  for(int iv = -7; iv < 8; iv++) {
    for(int iu = -7; iu < 8; iu++) {
      if(!IsCellValid.iu_iv_valid(0, 0, 0, iu, iv, m_sensorsize)) 
	continue;
      CellXY = TheCell.GetCellCoordinatesForPlots(0, 0, 0, iu, iv, m_sensorsize);
...
void RawHitPlotter::endJob()
{
  ...
  TFileDirectory hgpdir = fs->mkdir( "HighGainPedestal" );
  TFileDirectory lgpdir = fs->mkdir( "LowGainPedestal" );
  TFileDirectory hgndir = fs->mkdir( "HighGainNoise" );
  TFileDirectory lgndir = fs->mkdir( "LowGainNoise" );
  ...
  for( size_t it=0; it<NUMBER_OF_TIME_SAMPLES; it++ ){
    ...
    h->SetTitle(os.str().c_str());
    InitTH2Poly(*h);
    hgMeanMap.insert( std::pair<int,TH2Poly*>(it,h) );
    ...
    h->SetTitle(os.str().c_str());
    InitTH2Poly(*h);
    lgMeanMap.insert( std::pair<int,TH2Poly*>(it,h) );
    ...
    h->SetTitle(os.str().c_str());
    InitTH2Poly(*h);
    hgRMSMap.insert( std::pair<int,TH2Poly*>(it,h) );
  ...
  for( std::set< std::pair<int,HGCalTBDetId> >::iterator it=setOfConnectedDetId.begin(); it!=setOfConnectedDetId.end(); ++it ){
    ...
    channelInfo* cif=m_channelMap[iski*100+ichan];
    for( size_t it=0; it<NUMBER_OF_TIME_SAMPLES; it++ ){
      ...
      hgMeanMap[ it ]->Fill(iux , iuy, hgMean );
      lgMeanMap[ it ]->Fill(iux , iuy, lgMean );
      hgRMSMap[ it ]->Fill(iux , iuy, hgRMS );
      lgRMSMap[ it ]->Fill(iux , iuy, lgRMS );
    }
  }
}
