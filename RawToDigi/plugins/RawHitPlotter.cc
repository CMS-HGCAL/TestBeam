...
private:
  ...
  int m_layerID;
  int m_skirocID;
  std::vector<float> m_cmHigh;
  std::vector<float> m_cmLow;
...
void RawHitPlotter::beginJob()
{
  ...
  m_tree = fs->make<TTree>("commonMode", "Tree with common mode noise (one entry per event per skiroc)");
  m_tree->Branch("eventID", &m_evtID);
  m_tree->Branch("layerID", &m_layerID);
  m_tree->Branch("skirocID", &m_skirocID);
  m_tree->Branch("cmHighGain", &m_cmHigh);
  m_tree->Branch("cmLowGain", &m_cmLow);
