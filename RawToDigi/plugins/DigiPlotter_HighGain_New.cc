// -*- C++ -*-
//
// Package:    HGCal/DigiPlotter_HighGain_New
// Class:      DigiPlotter_HighGain_New
//
/**\class DigiPlotter_HighGain_New DigiPlotter_HighGain_New.cc HGCal/DigiPlotter_HighGain_New/plugins/DigiPlotter_HighGain_New.cc
...
class DigiPlotter_HighGain_New : public edm::one::EDAnalyzer<edm::one::SharedResources>
{

public:
	explicit DigiPlotter_HighGain_New(const edm::ParameterSet&);
	~DigiPlotter_HighGain_New();
...
//
DigiPlotter_HighGain_New::DigiPlotter_HighGain_New(const edm::ParameterSet& iConfig)
{
...
//Booking a "hexagonal" histograms to display the sum of Digis for NSAMPLES, in 1 SKIROC in 1 layer. To include all layers soon. Also the 1D Digis per cell in a sensor is booked here for NSAMPLES.
			...
			sprintf(title, "Sum of tdc counts per cell for ADC%i Layer%i Event%i", nsample, nlayers + 1,eee);
...
/*
                        ...
                        sprintf(title, "Sum of tdc counts per cell for ADC%i Layer%i Event%i Raw", nsample, nlayers + 1,eee);
...
*/
                        ...
                        sprintf(title, "Sum of tdc counts of cells  Event%i",eee);
...
DigiPlotter_HighGain_New::~DigiPlotter_HighGain_New()
{
...
void
DigiPlotter_HighGain_New::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
	using namespace edm;
	using namespace std;
...
//
			for(SKIROC2DigiCollection::const_iterator j = Coll.begin(); j != Coll.end(); j++) {
				...
				double iyy = (CellCentreXY.second < 0 ) ? (CellCentreXY.second + 0.0001) : (CellCentreXY.second - 0.0001);
                                        int nsample = 0;
/*
					if(flag_calib == 1) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, 0.5*(SKI[nsample].tdc() - 2*(Average_Pedestal_Per_Event/Cell_counter)));
                                        else h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event/Cell_counter)) );
*/
                                        if(flag_calib == 1){
//                                          h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, 0.5*(SKI[nsample].tdc() - 2*(Sum_Hist_cells_SKI1[event.id().event() - 1]->GetMean())));
//                                          h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, 0.5*(SKI[nsample].tdc()));
                                            
                                            if((iux <= 0.25 && iyy>= -0.25 ) || (iux < -0.5) ) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, 0.5*(SKI[nsample].tdc() - (Average_Pedestal_Calib_Cell_Event1/(Cell_counter1_Calib_Cell1))));
                                            else if((iux > -0.25 && iyy < -0.50 ) || (iux > 0.50)) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, 0.5*(SKI[nsample].tdc() - (Average_Pedestal_Calib_Cell_Event1/(Cell_counter1_Calib_Cell1))));

                                          }
                                        else{
//                                             h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Sum_Hist_cells_SKI1[event.id().event() - 1]->GetMean())) );
//                                             h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc()) );
//                                            if((iux <0.25 && iyy>= -0.25 ) || (iux < -0.25) ) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event1/(Cell_counter1))) );
                                            if(SKI.detid().cellType() == 3 && event.id().event() == 34) cout<<endl<<" iux= "<<iux<<" iyy= "<<iyy<<" ADC = "<<SKI[nsample].tdc()<<" Ped= "<< Average_Pedestal_Half_Cell_Event1/(Cell_counter1_Half_Cell1)<<endl;
                                            if((iux <= 0.25 && iyy>= -0.25 ) || (iux < -0.5) ){
                                               if( SKI.detid().cellType() == 0 || SKI.detid().cellType() == 5) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event1/(Cell_counter1))) );
                                               if(SKI.detid().cellType() == 2 || SKI.detid().cellType() == 3) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Half_Cell_Event1/(Cell_counter1_Half_Cell1))) ); 
                                               if(SKI.detid().cellType() == 1 || SKI.detid().cellType() == 4) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Calib_Cell_Event1/(Cell_counter1_Calib_Cell1))) );
                                              }
                                            else if((iux > -0.25 && iyy < -0.50 ) || (iux > 0.50)){
                                                    if( SKI.detid().cellType() == 0 || SKI.detid().cellType() == 5) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event2/(Cell_counter2))) );
                                                    if(SKI.detid().cellType() == 2 || SKI.detid().cellType() == 3) h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Half_Cell_Event2/(Cell_counter1_Half_Cell2))) );
...
void
DigiPlotter_HighGain_New::beginJob()
{
...
void
DigiPlotter_HighGain_New::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
DigiPlotter_HighGain_New::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
...
DEFINE_FWK_MODULE(DigiPlotter_HighGain_New);
