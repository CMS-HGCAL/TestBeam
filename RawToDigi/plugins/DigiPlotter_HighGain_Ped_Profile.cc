// -*- C++ -*-
//
// Package:    HGCal/DigiPlotter_HighGain_Ped_Profile
// Class:      DigiPlotter_HighGain_Ped_Profile
//
/**\class DigiPlotter_HighGain_Ped_Profile DigiPlotter_HighGain_Ped_Profile.cc HGCal/DigiPlotter_HighGain_Ped_Profile/plugins/DigiPlotter_HighGain_Ped_Profile.cc
...
class DigiPlotter_HighGain_Ped_Profile : public edm::one::EDAnalyzer<edm::one::SharedResources>
{

public:
	explicit DigiPlotter_HighGain_Ped_Profile(const edm::ParameterSet&);
	~DigiPlotter_HighGain_Ped_Profile();
...
//
DigiPlotter_HighGain_Ped_Profile::DigiPlotter_HighGain_Ped_Profile(const edm::ParameterSet& iConfig)
{
...
DigiPlotter_HighGain_Ped_Profile::~DigiPlotter_HighGain_Ped_Profile()
{
...
void
DigiPlotter_HighGain_Ped_Profile::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
	using namespace edm;
	using namespace std;
...
//////////////////////////////////Evaluate average pedestal per event to subtract out//////////////////////////////////
                        for(SKIROC2DigiCollection::const_iterator k = Coll.begin(); k != Coll.end(); k++) {
                                ...
                                int nsample = 0;
                                if((SKI_1[nsample].tdc() > ADC_TMP) && (SKI_1.detid()).cellType() == 0){
                                   ADC_TMP = SKI_1[nsample].tdc();
                                   iux_Max = iux;
                                   iyy_Max = iyy;
                                  }
                                if((iux <= -0.25 && iux >= -3.25) && (iyy <= -0.25 && iyy>= -5.25 ) && SKI_1.detid().cellType() == 0){
                                   Cell_counter1_Full++;
                                   Average_Pedestal_Per_Event1_Full += SKI_1[nsample].tdc();
                                   Sum_Hist_cells_SKI1[event.id().event() - 1]->Fill(SKI_1[nsample].tdc()); 
                                  }
     
                                if((iux >= 0.25 && iux <= 3.25) && (iyy <= -0.25 && iyy>= -5.25 ) && SKI_1.detid().cellType() == 0){
                                   Cell_counter2_Full++;
                                   Average_Pedestal_Per_Event2_Full += SKI_1[nsample].tdc();
                                  }

                                if((SKI_1.detid()).cellType() == 2 || (SKI_1.detid()).cellType() == 3 || (SKI_1.detid()).cellType() == 5 ){
                                    if((iux <= 0.25 && iyy>= -0.25 ) || (iux < -0.5) ){
                                          Cell_counter1_Half++;
                                          Average_Pedestal_Per_Event1_Half += SKI_1[nsample].tdc();
                                        }
                                    if((iux >= 0.25 && iyy <= -0.5) || (iux>= 0.5) ){
                                         Cell_counter2_Half++;
                                         Average_Pedestal_Per_Event2_Half += SKI_1[nsample].tdc();     
                                      }      
                                   }   
			...
			for(SKIROC2DigiCollection::const_iterator j = Coll.begin(); j != Coll.end(); j++) {
				...
				double iyy = (CellCentreXY.second < 0 ) ? (CellCentreXY.second + 0.0001) : (CellCentreXY.second - 0.0001);
                                        ...
                                        if(flag_calib == 1){
                                            ...
                                            if((iux <= 0.25 && iyy>= -0.25 ) || (iux < -0.5) )
                                                h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, 0.5*(SKI[nsample].tdc() - (Average_Pedestal_Per_Event1_Full/(Cell_counter1_Full))));
                                            else if((iux > -0.25 && iyy < -0.50 ) || (iux > 0.50))
                                                h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, 0.5*(SKI[nsample].tdc() - (Average_Pedestal_Per_Event2_Full/(Cell_counter2_Full))));
                                          }
                                        else{
                                             if((SKI.detid()).cellType() == 2 || (SKI.detid()).cellType() == 3 || (SKI.detid()).cellType() == 5 ){
                                                   if((iux <= 0.25 && iyy>= -0.25 ) || (iux < -0.5) ){
                                                       h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event1_Half/(Cell_counter1_Half))) );
                                                     }
                                                  else if((iux > -0.25 && iyy < -0.50 ) || (iux > 0.50))
                                                       h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event2_Half/(Cell_counter2_Half))) );
           }
                                             else if((SKI.detid()).cellType() == 0 ){
                                                      if((iux <= 0.25 && iyy>= -0.25 ) || (iux < -0.5) ){ 
                                                          h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event1_Full/(Cell_counter1_Full))) );
                                                         if(((iux < (iux_Max+2)  && iux > (iux_Max -2)) && (iyy < (iyy_Max +2) && iyy > (iyy_Max -2))) ){
//                                                            Sum_Cluster_ADC->Fill((SKI[nsample].tdc() - (Average_Pedestal_Per_Event1_Full/(Cell_counter1_Full))));
                                                            Sum_Cluster_Tmp+= (SKI[nsample].tdc() - (Average_Pedestal_Per_Event1_Full/(Cell_counter1_Full))); 
                                                            iux_CG += iux*(SKI[nsample].tdc() - (Average_Pedestal_Per_Event1_Full/(Cell_counter1_Full)));
                                                            iyy_CG += iyy*(SKI[nsample].tdc() - (Average_Pedestal_Per_Event1_Full/(Cell_counter1_Full))); 
                                                            CG_counter++;
                                                           }
                                                        }
                                                      else if((iux > -0.25 && iyy < -0.50 ) || (iux > 0.50)){
                                                               h_digi_layer[nsample][n_layer - 1][event.id().event() - 1]->Fill(iux , iyy, (SKI[nsample].tdc() - (Average_Pedestal_Per_Event2_Full/(Cell_counter2_Full))) );
                                                               if((iux < (iux_Max+2)  && iux > (iux_Max -2)) && (iyy < (iyy_Max +2) && iyy > (iyy_Max -2)) ){
//                                                                   Sum_Cluster_ADC->Fill((SKI[nsample].tdc() - (Average_Pedestal_Per_Event2_Full/(Cell_counter2_Full))));
                                                                   Sum_Cluster_Tmp+= (SKI[nsample].tdc() - (Average_Pedestal_Per_Event2_Full/(Cell_counter2_Full)));
                                                                   iux_CG += iux*(SKI[nsample].tdc() - (Average_Pedestal_Per_Event2_Full/(Cell_counter2_Full)));
                                                                   iyy_CG += iyy*(SKI[nsample].tdc() - (Average_Pedestal_Per_Event2_Full/(Cell_counter2_Full)));
                                                                   CG_counter++;
                                                                 }
                                                             }
                                                     }                                  
...
void
DigiPlotter_HighGain_Ped_Profile::beginJob()
{
...
void
DigiPlotter_HighGain_Ped_Profile::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
DigiPlotter_HighGain_Ped_Profile::fillDescriptions(edm::ConfigurationDescriptions& descriptions)
{
...
DEFINE_FWK_MODULE(DigiPlotter_HighGain_Ped_Profile);
