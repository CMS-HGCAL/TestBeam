...
process.source = cms.Source("HGCalTBWireChamberSource",
    ...
    fileNames = cms.untracked.vstring(["file:%s" % file[0] for file in options.inputFiles]),
    timingFileNames = cms.vstring(["%s" % file[0] for file in options.timingFiles]),
    sumTriggerTimes = cms.vint32([sumTrigger[0] for sumTrigger in options.sumTriggerTimeStamps]),
    skipFirstNEvents = cms.vint32([skipFirstNEvents[0] for skipFirstNEvents in options.skipFirstNEvents]),
    triggerCountOffsets = cms.vint32([triggerCountOffset[0] for triggerCountOffset in options.triggerCountOffsets]),
    ...
    allowForTDCEventSkipping = cms.vint32([skipTDCTrigger[0] for skipTDCTrigger in options.skipTDCTriggers]),
    setupIDs = cms.vint32([setupID[0] for setupID in options.setupIDs]),
