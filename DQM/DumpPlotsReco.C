...
void DumpPlotsReco(TString inputFileName, TString outputFolder, Int_t runNumber, Int_t nSpills) {
  ...
  std::cout << "DumpPlotsReco called for inputFileName: " << inputFileName << ", outputFolder: " << outputFolder << ", runNumber: " << runNumber << std::endl;
  ...
  histogramName = Form("Noise_2D_Profile_Layer");
  TH2F* h_Noise_2D_Profile_Layer  = (TH2F*) inputFile->FindObjectAny(histogramName);
  ...
  histogramName = Form("AllCells_Ped");
  TH1F* h_AllCells_Ped  = (TH1F*) inputFile->FindObjectAny(histogramName);
  ...
  histogramName = Form("AllCells_CM");
  TH1F* h_AllCells_CM  = (TH1F*) inputFile->FindObjectAny(histogramName);
  ...
  for(int layerCounter=0; layerCounter < nLayers; ++layerCounter){
    histogramName = Form("Full_Cell_Layer_%d", layerCounter);
    TH1F* h_Full_Cell  = (TH1F*) inputFile->FindObjectAny(histogramName);
    ...
    histogramName = Form("Half_Cell_Layer_%d", layerCounter);
    TH1F* h_Half_Cell  = (TH1F*) inputFile->FindObjectAny(histogramName);
    ...
    histogramName = Form("MB_Cell_Layer_%d", layerCounter);
    TH1F* h_MB_Cell  = (TH1F*) inputFile->FindObjectAny(histogramName);
    ...
    histogramName = Form("Calib_Pads_Layer_%d", layerCounter);
    TH1F* h_Calib_Pads  = (TH1F*) inputFile->FindObjectAny(histogramName);
    ...
    histogramName = Form("Merged_Cell_Layer_%d", layerCounter);
    TH1F* h_Merged_Cell  = (TH1F*) inputFile->FindObjectAny(histogramName);
    ...
    //     histogramName = Form("Ski_%d_Channel_%d_Layer_%d", skirocCounter, channelCounter, layerCounter);
    //     TH1F* h_Ski  = (TH1F*) inputFile->FindObjectAny(histogramName);
    //     if (not(NULL == h_Ski)) {
    //       h_Ski->Draw();
  ...
  histogramName = Form("Covar_hist");
  TH2F* h_Covar_hist  = (TH2F*) inputFile->FindObjectAny(histogramName);
  ...
  histogramName = Form("Correl_hist");
  TH2F* h_Correl_hist  = (TH2F*) inputFile->FindObjectAny(histogramName);
  ...
  histogramName = Form("DiffIJ_hist");
  TH2F* h_DiffIJ_hist  = (TH2F*) inputFile->FindObjectAny(histogramName);
  ...
  histogramName = Form("CG_X");
  TH1F* h_CG_X  = (TH1F*) inputFile->FindObjectAny(histogramName);
  if (not(NULL == h_CG_X)) {
    h_CG_X->Draw();
    outputPlotName = (outputFolder + Form("/")) + histogramName + Form("_%06d.png", runNumber);
    plotCanvas->SaveAs(outputPlotName);
  ...
  histogramName = Form("CG_Y");
  TH1F* h_CG_Y  = (TH1F*) inputFile->FindObjectAny(histogramName);
  if (not(NULL == h_CG_Y)) {
    h_CG_Y->Draw();
    outputPlotName = (outputFolder + Form("/")) + histogramName + Form("_%06d.png", runNumber);
    plotCanvas->SaveAs(outputPlotName);
  ...
  histogramName = Form("Sum_Cluster_ADC");
  TH1F* h_Sum_Cluster_ADC  = (TH1F*) inputFile->FindObjectAny(histogramName);
  ...
  histogramName = Form("Sum_Cluster_Max");
  TH1F* h_Sum_Cluster_Max  = (TH1F*) inputFile->FindObjectAny(histogramName);
  ...
  for(int layerCounter=1; layerCounter <= nLayers; ++layerCounter){
    histogramName = Form("FullLayer_RecHits_Layer%d_Summed", layerCounter);
    TH1F* h_FullLayer_RecHits  = (TH1F*) inputFile->FindObjectAny(histogramName);
  ...
  for (int eventCenturyCounter = 0; eventCenturyCounter < int(nEvents/100); ++eventCenturyCounter) {
    ...
    int layerNumber = 1+int(eventNumber/EVENTSPERSPILL);
    histogramName = Form("FullLayer_ADC0_Layer%d_Event%d", layerNumber, eventNumber);
    TH2Poly* h_EventDisplay  = (TH2Poly*) inputFile->FindObjectAny(histogramName);
