...
class MillepedeBinaryWriter_2016 : public edm::one::EDAnalyzer<edm::one::SharedResources> {
	public:
		explicit MillepedeBinaryWriter_2016(const edm::ParameterSet&);
		~MillepedeBinaryWriter_2016();
...
MillepedeBinaryWriter_2016::MillepedeBinaryWriter_2016(const edm::ParameterSet& iConfig) {
...
MillepedeBinaryWriter_2016::~MillepedeBinaryWriter_2016() {
	return;
}

// ------------ method called for each event  ------------
void MillepedeBinaryWriter_2016::analyze(const edm::Event& event, const edm::EventSetup& setup) {
	...
	if (useMWCReference) {
	  ...
	  if (rd->booleanUserRecords.has("hasValidDWCMeasurement")&&!rd->booleanUserRecords.get("hasValidDWCMeasurement")) return;	
		if (MWCQualityCut) {
...
void MillepedeBinaryWriter_2016::beginJob() {	
}

void MillepedeBinaryWriter_2016::endJob() {
	if (mille != NULL) {
		mille->kill();
		delete mille;
	}
...
void MillepedeBinaryWriter_2016::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(MillepedeBinaryWriter_2016);