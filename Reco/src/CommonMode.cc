...
void CommonMode::EvaluateMedianPerLayer( edm::Handle<HGCalTBRawHitCollection> hits )
{
  ...
  for( int ilayer=0; ilayer<HGCAL_TB_GEOMETRY::NUMBER_OF_HEXABOARD; ilayer++ ){
    for( int it=0; it<NUMBER_OF_TIME_SAMPLES; it++ ){
      cmMapHG[NUMBER_OF_TIME_SAMPLES*ilayer + it].reserve(128);
      cmMapLG[NUMBER_OF_TIME_SAMPLES*ilayer + it].reserve(128);
      cmMapHGHalf[NUMBER_OF_TIME_SAMPLES*ilayer + it].reserve(128);
      cmMapLGHalf[NUMBER_OF_TIME_SAMPLES*ilayer + it].reserve(128);
    }
  }
  for( auto hit : *hits ){
    HGCalTBElectronicsId eid( _emap.detId2eid(hit.detid().rawId()) );
    if( !_emap.existsEId(eid) ) continue;
    int ilayer=hit.detid().layer()-1;
    for( size_t it=0; it<NUMBER_OF_TIME_SAMPLES; it++ ){
      if( hit.detid().cellType()==0 || hit.detid().cellType()==4 ) {
	cmMapHG[NUMBER_OF_TIME_SAMPLES*ilayer+it].push_back(hit.highGainADC(it));
	cmMapLG[NUMBER_OF_TIME_SAMPLES*ilayer+it].push_back(hit.lowGainADC(it));
      }
      else if( hit.detid().cellType()!=5 ){
	cmMapHGHalf[NUMBER_OF_TIME_SAMPLES*ilayer+it].push_back(hit.highGainADC(it));
	cmMapLGHalf[NUMBER_OF_TIME_SAMPLES*ilayer+it].push_back(hit.lowGainADC(it));
      }
    }
  }
  ...
  for( int ilayer=0; ilayer<HGCAL_TB_GEOMETRY::NUMBER_OF_HEXABOARD; ilayer++ ){
    for( int it=0; it<NUMBER_OF_TIME_SAMPLES; it++ ){
      int index=NUMBER_OF_TIME_SAMPLES*ilayer+it;
      std::sort( cmMapHG[index].begin(),cmMapHG[index].end() );
      std::sort( cmMapLG[index].begin(),cmMapLG[index].end() );
      std::sort( cmMapHGHalf[index].begin(),cmMapHGHalf[index].end() );
      std::sort( cmMapLGHalf[index].begin(),cmMapLGHalf[index].end() );
	...
	uint16_t size = cmMapHG[index].size();
	_cmMap[ilayer].fullHG[it] = size%2==0 ? cmMapHG[index][size/2-1] : cmMapHG[index][size/2] ;
	_cmMap[ilayer].outerHG[it] = _cmMap[ilayer].fullHG[it];
	_cmMap[ilayer].mergedHG[it] = _cmMap[ilayer].fullHG[it]*1.5;
	_cmMap[ilayer].fullLG[it] = size%2==0 ? cmMapLG[index][size/2-1] : cmMapLG[index][size/2] ;
	_cmMap[ilayer].outerLG[it] = _cmMap[ilayer].fullLG[it];
	_cmMap[ilayer].mergedLG[it] = _cmMap[ilayer].fullLG[it]*1.5;
	size = cmMapHGHalf[index].size();
	_cmMap[ilayer].halfHG[it] = size%2==0 ? cmMapHGHalf[index][size/2-1] : cmMapHGHalf[index][size/2] ;
	_cmMap[ilayer].mouseBiteHG[it] = _cmMap[ilayer].halfHG[it]*topo.Cell_Area(3)/topo.Cell_Area(2);
	_cmMap[ilayer].halfLG[it] = size%2==0 ? cmMapLGHalf[index][size/2-1] : cmMapLGHalf[index][size/2] ;
	_cmMap[ilayer].mouseBiteLG[it] = _cmMap[ilayer].halfLG[it]*topo.Cell_Area(3)/topo.Cell_Area(2);
      }
    }
  }
}
