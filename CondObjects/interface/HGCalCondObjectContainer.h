...
public:
  ...
  virtual size_t denseIndexFor(DetId rawDetId, uint64_t scheme) const { return denseIndexFor(rawDetId.rawId(),scheme); }  
};
...
public:
  ...
  void setNumberingScheme(const HGCalCondObjectNumberingScheme* scheme) { p_numbering=scheme; }
  void setNumberingScheme(const HGCalCondObjectNumberingScheme* scheme, uint64_t code) { p_numbering=scheme; m_numbering_code=code;}
  const HGCalCondObjectNumberingScheme* getNumberingScheme() const { return p_numbering; }
  uint64_t schemeCode() const { return m_numbering_code; }
  
  bool exists(DetId id) const { return indexOf(id)!=HGCalCondObjectNumberingScheme::INVALID; }
protected:
...
public:
  ...
  const Item* get(DetId id) const { size_t i=indexOf(id); return (i==HGCalCondObjectNumberingScheme::INVALID)?(0):(&(m_payload[i])); }
  size_t size() const { return m_payload.size(); }
  const Item& get(size_t i) const { return m_payload[i]; }
