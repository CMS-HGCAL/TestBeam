...
int main(int argc, char *argv[]) {
  ...
  if (!mapfile_in.empty()) {
    ...
    printf("Reading electronics map from '%s'...",mapfile_in.c_str());  fflush(stdout);
    bool ok=io.load(mapfile_in,emap);
    if (ok) printf("OK\n"); else printf("ERROR\n");
    printf("Rewriting electronics map to '%s.out'...",mapfile_in.c_str());  fflush(stdout);
  ...
  if (!valuefile_in.empty()) {
    ...
    printf("Reading single values from '%s'...",valuefile_in.c_str());  fflush(stdout);
    bool ok=io.load(valuefile_in,values);
    if (ok) printf("OK\n"); else printf("ERROR\n");
    printf("Rewriting single values to '%s.out'...",valuefile_in.c_str());  fflush(stdout);
