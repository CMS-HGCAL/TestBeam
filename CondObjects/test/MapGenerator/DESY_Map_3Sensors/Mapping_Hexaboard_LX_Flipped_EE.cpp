...
//  fs.open("/afs/cern.ch/work/r/rchatter/May_Test_Beam_Test/CMSSW_8_0_1/src/HGCal/CondObjects/data/map_FNAL_SB1.txt");
  fs.open("map_CERN_Hexaboard_September_L_FOO.txt");
...
int hike = 0;
for(int ILayer = FOO; ILayer <= FOO; ILayer++){
   hike = 4*(ILayer - 1);
   for(int sk = 1; sk <= 4; sk++){

       for(int iii = 0; iii < 68; iii++){
           if( iii >= SKI_Channels[sk - 1] ) continue;
                    if(sk == 1) Print_Map_Line(sk + hike,Channel_1[iii],ILayer,sensor_iu,sensor_iv,(IU_1[iii]+IV_1[iii]),-IV_1[iii],Cell_Type(Channel_1[iii], 1));
                    if(sk == 2) Print_Map_Line(sk + hike,Channel_4[iii],ILayer,sensor_iu,sensor_iv,(IU_4[iii]+IV_4[iii]),-IV_4[iii],Cell_Type(Channel_4[iii], 4));
		    if(sk == 3) Print_Map_Line(sk + hike,Channel_3[iii],ILayer,sensor_iu,sensor_iv,(IU_3[iii]+IV_3[iii]),-IV_3[iii],Cell_Type(Channel_3[iii], 3));
		    if(sk == 4) Print_Map_Line(sk + hike,Channel_2[iii],ILayer,sensor_iu,sensor_iv,(IU_2[iii]+IV_2[iii]),-IV_2[iii],Cell_Type(Channel_2[iii], 2));
...
int Cell_Type(int channel, int skiroc){
	...
	for(int iii=0;iii<number_of_outer_calib_pads;iii++)
	   ...
	   for(int iii=0;iii<number_of_outer_calib_pads;iii++)
               if(channel == Map_Merged_Outer_Calib_Pad_SK3[iii]) return 4;
